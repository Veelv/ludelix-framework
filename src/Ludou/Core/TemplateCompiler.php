...
class TemplateCompiler implements TemplateCompilerInterface
{
    ...
    public function compile(string $template, array $functions = [], array $filters = []): string
    {
        ...
        if ($this->fluidHook !== null) {
            try {
                if ($logger) $logger->debug('[Ludou] Aplicando Fluid beforeRender...');
                $template = $this->fluidHook->beforeRender($template);
                if ($logger) $logger->debug('[Ludou] Fluid beforeRender aplicado com sucesso');
            } catch (\Throwable $e) {
                if ($logger) $logger->error('[Ludou] Error processing Fluid beforeRender: ' . $e->getMessage());
            }
        } else {
            if ($logger) $logger->debug('[Ludou] FluidHook é NULL - aplicando fallback manual');
            ...
            if ($logger) $logger->debug('[Ludou] Fallback manual aplicado');
        }
        ...
        if (preg_match('/\|/', $expression)) {
            ...
            foreach ($parts as $i => $part) {
                ...
                if (preg_match('/^([a-zA-Z_][a-zA-Z0-9_]*)\s*\((.*)\)$/', $part, $matches)) {
                    ...
                    $argsPHP = [];
                    foreach ($argsList as $a) $argsPHP[] = $a;
                    foreach ($namedArgs as $k => $v) $argsPHP[] = "'{$k}' => {$v}";
                    $argsPHPstr = implode(', ', $argsPHP);
                    $ludouFunctions = [
                        'range', 'count', 'merge', 'sort', 'filter', 'keys', 'values', 'now', 'date', 'min', 'max', 'random', 'sum', 'avg', 'empty', 'inArray', 'isNumeric', 'isString', 'isArray', 'startsWith', 'endsWith', 'split', 'length', 'json', 'type'
                    ];
    ...
    protected function errorMessage($msg, $expression = '', $template = '', $offset = 0) {
        ...
        if ($line) $html .= "<br><b>Linha:</b> $line, <b>Coluna:</b> $col";
        if ($debug && $context) $html .= $context;
        $html .= "</div>";
        return $html;
    }
}