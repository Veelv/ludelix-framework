 ...
 */
class RouterComplete implements RouterInterface
{
    ...
    {
        return $this->addRoute(array_map('strtoupper', $methods), $path, $handler, $options);
    }

    public function any(string $path, mixed $handler, array $options = []): RouteInterface
    {
        return $this->addRoute(['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS', 'HEAD'], $path, $handler, $options);
    }
    ...
    {
        $this->databaseParser->parse($criteria, $this);
        return $this;
    }
